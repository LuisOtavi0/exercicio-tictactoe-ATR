...
Este repositório contém a solução para um problema de **Programação Concorrente** proposto pelo professor Leandro Abreu, na disciplina de Automação em Tempo Real da UFMG. O objetivo é gerenciar o acesso a um recurso compartilhado (tabuleiro) e coordenar a alternância de turnos entre dois agentes (threads) utilizando C++.
...
* **Sincronização:**
   * `std::mutex`: Para garantir exclusão mútua no acesso ao tabuleiro.
...
# Compilação
g++ -pthread tic_tac_toe.cpp -o tictactoe
